...
//--------------------- Elham - MusicHistoryServiceUnitTest --------------
@ExtendWith(MockitoExtension.class)
class MusicHistoryServiceUnitTest {
    ...
    @InjectMocks
    private MusicHistoryService mockMusicHistoryService;

    String url = "https://youtu.be/-uQi0vJK9lk";
    String userId = "1b2d25d9-0ac1-494d-8208-6b15b6430f3a";
    String token = "access-token";

    @BeforeEach
    void setUp() {
        ReflectionTestUtils.setField(mockMusicHistoryService, "musicServiceUrl", "http://songservice:8080");
    }

    @Test
    void registerMusicHistory_ShouldRegisterMusictHistoryInUserActivityRepository() {
        //Arrange
        MediaResponseDTO response = new MediaResponseDTO();
        response.setMediaId(UUID.fromString("71c74694-e496-48e2-8b4a-9409c05eaaae"));
        response.setGenres(List.of("Rock", "Punk", "Alternative"));
        ...
        //Act
        Object result = mockMusicHistoryService.registerMusicHistory(url, userId, token);
        //Assert
        UserActivity activity = (UserActivity) result;
        assertThat(activity.getUserId()).isEqualTo("1b2d25d9-0ac1-494d-8208-6b15b6430f3a");
        assertThat(activity.getMediaId()).isEqualTo(UUID.fromString("71c74694-e496-48e2-8b4a-9409c05eaaae"));
        assertThat(activity.getMediaType()).isEqualTo(MediaType.MUSIC.name());
        assertThat(activity.getGenreName()).isEqualTo(List.of("Rock", "Punk", "Alternative"));
    ...
    @Test
    void registerMusicHistory_ShouldThrowExceptionWhenHistoryRegistrationFails() {
        //Arrange
        when(mockRestTemplate.exchange(anyString(), eq(HttpMethod.GET),
                any(HttpEntity.class), eq(MediaResponseDTO.class)))
        ...
        RuntimeException ex = assertThrows(RuntimeException.class, () ->
                mockMusicHistoryService.registerMusicHistory(url, userId, token));
        assertThat(ex.getMessage()).contains("Error while registering music history");
