...
*/
    ...
    public List<UUID> getLikedMediaList(UUID userId) {
        try {
            ResponseEntity<List<UUID>> response = restClient.put()
                    .uri(userLikedApiUrl, userId)
                    .retrieve()
            ...
            try {
                ...
                throw new IllegalStateException(
                        String.format("Failed to retrieve user information. Status %s, %s, Path:%s",
                                status, message, path), e);
            } catch (IOException parseEx) {
                throw new IllegalStateException("Failed to retrieve user dislike information. Status=" + status + " body=" + body, e);
            }
    ...
    public List<UUID> getDislikedMediaList(UUID userId) {
        try {
            ResponseEntity<List<UUID>> response = restClient.put()
                    .uri(userDislikedApiUrl, userId)
                    .retrieve()
                    .toEntity(new ParameterizedTypeReference<List<UUID>>() {});
            if (response.getStatusCode().is2xxSuccessful()) {
                return response.getBody();
